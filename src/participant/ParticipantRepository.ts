...
export interface ParticipantRepository {
    ...
    getAllGroupedByDateRegistered(): mongoose.Aggregate<
        DateRegisteredAggregation[]
    >;
}

export default class ParticipantRepositoryImpl
    implements ParticipantRepository
{
    ...
    public getAllGroupedByDepartment = (): mongoose.Aggregate<
    ...
    public getAllGroupedByDateRegistered = (): mongoose.Aggregate<
        DateRegisteredAggregation[]
    > => {
    ...
    public saveMatchesToPreviousMatches = async (
        matches: Array<Match>
    ): Promise<unknown[]> => {
        const promises: Array<Promise<unknown>> = [];
        for (const { participant_1: p1, participant_2: p2 } of matches) {
            p1.matches.push(p2.email);
            p2.matches.push(p1.email);

            promises.push(p1.save(), p2.save());
        }
